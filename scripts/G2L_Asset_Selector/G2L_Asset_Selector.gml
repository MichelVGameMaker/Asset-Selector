...
function G2L_Asset_Selector(_type = asset_object, _type_change = false) constructor
{
	// everything is private
	static id_gen = 1;
	__id          = "##G2L_L15T" + string(id_gen++)			// id, to avoid collision in imguigml identifier, in case multiple selectors are created.
	// assets list
	...
	static __update_page   = function()						// Updates the array storing all asset_names (to be used in imguigml widget), accoding to sorting and filtering rules.
	{
		...
		if list_filter == "" 
		{
			array_copy(page_names, 0, _names, list_page * G2L_ITEMS_PER_PAGE, G2L_ITEMS_PER_PAGE);
		}
		else
		{
			...
			repeat (_len)
			{
				...
				if string_pos(string_upper(list_filter), string_upper(_name_i)) 
				{
					...
					if _j >= G2L_ITEMS_PER_PAGE break
				}
				++_i;      
			}
		}
	...
	static imguigml_step  = function(_nb_entries)			// Processes scan instructions and imguigml instructions for the widget to be displayed. Intended to be called within your imguigml isntructions like a call to imguigml_list_box().
	{
		...
		if type_change
		{
			...
			imguigml_same_line();
			_input = imguigml_combo(__id+"TYPE", asset_code, G2L_asset_names);
			if _input[0]
			{
				var _asset_types = G2L_asset_types; // due to the macro being in-line. I need this to access with [i]
				asset_code = _input[1];
